...
Claude Code stores conversation logs as JSONL files in `~/.claude/projects/`. Each project gets a directory named after its filesystem path (e.g., `-Users-travis-GitHub-myapp`), containing one `.jsonl` file per session.
